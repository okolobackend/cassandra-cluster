...
4. Для машины А активируем **Неразборчивый режим (Promiscuous Mode): "Разрешить всё"**
...
4. Создаем виртуальный интерфейс, чтобы хост мог общаться с контейнерами. Делаем по науке.
  ...
  1. Создаем файл с настройками сетевого интерфейса 
    ...
    ```
    #!/bin/bash
    sudo ip link add ipvlan0 link enp0s3 type ipvlan mode l2
...
5. Создаем ssh ключ и копируем в контекст сборки

   ```
   ssh-keygen -t rsa -f ~/.ssh/id_rsa -N ""
   sudo cp ~/.ssh/id_rsa.pub /opt/.ssh/authorized_keys
   ```

6. Запускаем кластер/отключаем кластер
...
1. Установка возможности перенаправления для сети

   ```
   sudo sysctl -w net.ipv4.ip_forward=1
   ```

2. Получение мак-адреса для хоста Б(при macvlan)
...
3. Проверка текущих маршрутов на хосте А

   ```
   ip route show | grep 192.168.1.
   ```

4. Перехват трафика на машине А.

   ```
   sudo tcpdump -i enp0s3 host 192.168.1.198
   ```

5. Добавление явного маршрута через интерфейс с указанием источника

   ```
   sudo ip route add 192.168.1.200/29 dev enp0s3 src 192.168.1.197
   ```

6. Включение proxy ARP (если ничего не помогает)

   ```
   sudo sysctl -w net.ipv4.conf.enp0s3.proxy_arp=1
   ```

7. Проверка iptables на хосте и смена политики **FORWARD** 
